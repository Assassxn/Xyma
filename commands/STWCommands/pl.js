        ...
        if (args.length === 0 || args[0].startsWith("<")) {
            ...
            await data.links.forEach(async (linkedAcc, index) => {
                if (linkedAcc.discordID === targetID) {
                    found = true;
                    await returnPl(linkedAcc.accountID).then(async (pls) => {
                ...
                if (!found && index === data.links.length - 1) {
                    message.channel.send(
                        `${message.guild.members.cache.get(
                            targetID
                        )}, you need to link your epic account first. To do that head over to <#871364695049797652> and react with <:epicgames:871366292232351784>`
                    );
                }
            });
        }
